...
//Inicio da Função que cadastra usuario no onesignal
function userOneSignal() {
    var userIdForOne = localStorage.getItem('userId');
  // URL da API do OneSignal
  const url = "https://api.onesignal.com/apps/7bfc9cb0-b251-4b3a-be5c-be82c1a143e2/users";

  // Obter o timestamp atual
  const currentTimestamp = Math.floor(Date.now() / 1000); // Converter para segundos

  // Configurações da requisição
  const options = {
    method: "POST", // Método HTTP
    headers: {
      accept: "application/json", // Aceitar respostas no formato JSON
      "content-type": "application/json", // Enviar corpo da requisição como JSON
      Authorization: "Basic os_v2_app_pp6jzmfskfftvps4x2bmdikd4l6un3yoc3kepsvykmyx6lwcma2bukuosuadxl4cvqquyztlfvukfgoy7tfgmvnvhsbqr64xhv7uk7i", // Substitua pela sua chave de API
    },
    body: JSON.stringify({
      // Corpo da requisição
      properties: {
        language: "en", // Idioma
        timezone_id: "America/Los_Angeles", // Fuso horário
        lat: 90, // Latitude
        long: 135, // Longitude
        country: "US", // País
        first_active: currentTimestamp, // Primeiro acesso como o timestamp atual
        last_active: currentTimestamp, // Última atividade como o timestamp atual
      },
      identity: { external_id: userIdForOne }, // Identidade com external_id
    }),
  };
  ...
  fetch(url, options)
    .then((res) => {
      ...
      return res.json(); // Converter resposta para JSON
    })
    .then((json) => {
      console.log("Resposta da API:", json); // Exibir a resposta no console
    })
    .catch((err) => {
      console.error("Erro ao fazer a requisição:", err); // Exibir erros no console
    });
}
