...
// Correção da função buscarProduto em funcoes.js
...
fetch(apiServerUrl, options)
  ...
  .then((responseJson) => {
    ...
    if (responseJson.status === "success" && responseJson.data.status === "success") {
      ...
      //ALIMENTAR COM OS VALORES DO ITEM DA TELA DE DETALHES
      ...
      // *** ATUALIZAR OS DADOS DO POPUP DE COMPARTILHAMENTO AQUI ***
      // Sempre que carregar um novo produto, atualiza os dados do popup
      $("#imagemShare").attr('src', fotoPrincipal);
      $("#nomeShare").html(detalhes.nome.toUpperCase());
      $("#precoShare").html(formatarMoeda(detalhes.preco));
      
      // LIMPAR O QRCODE E LINKS ANTERIORES
      $("#qrcode").empty();
