...
var app = new Framework7({
  ...
  function updateGreeting() {
    ...
    // Obter o elemento h1 na seção de boas-vindas
    const greetingEl = document.querySelector('.welcome-section h1');
    if (greetingEl) {
      const userName = document.getElementById('nomeUsuarioHome').textContent;
      greetingEl.innerHTML = `${greeting}, <span id="nomeUsuarioHome">${userName}</span>!`;
    }
  }
  
  // Exibir as iniciais do usuário
  ...
  // Aplicar animação nos cards do dashboard
  function applyCardAnimations() {
    const cards = document.querySelectorAll('.card-dash, .wallet-card, .referral-card');
    cards.forEach((card, index) => {
      card.style.animationDelay = `${0.1 * (index + 1)}s`;
    });
  }
  
  // Adicionar efeito ao botão de atualizar
  function setupUpdateButton() {
    const updateBtn = document.getElementById('updateData');
    if (updateBtn) {
      updateBtn.addEventListener('click', function() {
        // Adicionar classe para efeito visual de rotação
  ...
  // Aplicar os efeitos quando a página for carregada
  ...
  // Verificar se estamos na página de gestão
  if (window.location.href.includes('/gestao/') || 
      document.querySelector('.welcome-section')) {
    // Inicializar com pequeno atraso para garantir que os elementos estão carregados
    setTimeout(initWelcomeSection, 100);
  }
  
  // Adicionar CSS para a animação de rotação do botão de atualizar
